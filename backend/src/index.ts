export default {
  ...
  async bootstrap({ strapi }) {
    ...
    const updatePermissions = async (role, permissions) => {
      ...
      const newPermissions = permissionsToEnable.filter(
        (p) => !currentPermissions.find((cp) => cp.action === p.action),
      );

      if (newPermissions.length > 0) {
        strapi.log.info(
          `Adding ${newPermissions.length} permissions to role ${role.type}`,
        );
        await Promise.all(
  ...
  try {
    ...
    const updatedSettings = {
      ...
      google: {
        ...
        callback:
          process.env.GOOGLE_REDIRECT_URI || `${FRONTEND_URL}/callback/google`,
        scope: ["email", "profile"],
      },
    };
